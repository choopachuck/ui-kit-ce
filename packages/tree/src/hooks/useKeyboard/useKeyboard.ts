...
export const useKeyboard = <TItem = TreeItem>({
  ...
  const handleKeyDown = (event: React.KeyboardEvent<HTMLUListElement>) => {
     ...
     *
     * 1) Когда фокус находится на закрытом узле, узел открывается; фокус не перемещается;
     *
     * 2) Когда фокус находится на открытом узле, фокус перемещается на первый дочерний узел;
     *
     * 3) Когда фокус находится на конечном узле (узле дерева без дочерних узлов), ничего не происходит.
     */
    if (
      !disableExpand &&
      !!focusedItemKey &&
     ...
     *
     * 1) Когда фокус находится на открытом узле, узел закрывается;
     *
     * 2) Когда фокус находится на дочернем узле, который также является конечным узлом или закрытым узлом,
     *    фокус перемещается на его родительский узел;
     *
     ...
     *
     * Перемещение фокуса на следующий узел, на котором можно фокусироваться, но не открывая и не закрывая его
     */
     ...
     *
     * Перемещение фокуса на предыдущий узел, на котором можно фокусироваться, но не открывая и не закрывая его
     */
     ...
     *
     * Выполнение действия по-умолчанию для текущего узла. Для родительских узлов действие открывает или закрывает узел.
     * В случае кейса с единичным выбором если у узла нет дочерних узлов, выбирается текущий узел.
     */
     ...
     *
     * Изменяет состояние выбора узла
     *
     * Если пробел был нажат с зажатой клавишей SHIFT, то выбирается диапазон значений
     * от последнего выбранного узла до текущего (при selectMode === directory)
     */
    if (
     ...
     *
     * Изменяет состояние выбора у следующего узла
     */
    if (
     ...
     *
     * Изменяет состояние выбора у предыдущего узла
     */
    if (
    ...
    if (selectable && selectMode === TreeSelectMode.directory) {
       ...
       *
       * Выбор всех узлов, начиная с текущего и до первого (при selectMode === directory)
       */
      if (
       ...
       *
       * Выбор всех узлов, начиная с текущего и до последнего (при selectMode === directory)
       */
      if (
       ...
       *
       * Выбор всех узлов (при selectMode === directory)
       */
      if (
