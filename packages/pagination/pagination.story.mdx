...
Компонент `Pagination` предоставляет хук `usePagination`,
который позволяет осуществлять пользовательскую настройку пагинации.
Использование данного хука дает возможность изменять внешний вид пагинации, при этом сохраняя все ее функциональные возможности.
